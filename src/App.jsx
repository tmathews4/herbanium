...
/* ──────────────────────────────────────────────────────────────
   ...
   beat after first arrival (instead of throwing the user straight
   into a spotlight), and gates the whole guided-tour system on an
   explicit yes. Declining is quiet and reversible — "replay tour"
...
const TourOfferCard = ({ onYes, onNo }) => (
  <div data-testid="tour-offer" style={{
    ...
    <div style={{ fontFamily: ff.sans, fontSize: 12.5, color: theme.inkSoft, lineHeight: 1.5, marginBottom: 12 }}>
      We can point out what things do as you visit each screen.
  ...
  return (
    ...
    <div ref={barRef} data-testid="main-menu" style={{
      ...
      {/* Slot for the blend screen's brew controls, which mount into
          ...
          scrolling page — the same reasoning that puts the sub-tabs
          here — but the temp/steep state they drive lives deep inside
          Compose, and hauling it up to App just to render it in the
...
export default function App() {
  ...
  // Tours are opt-in: null = not asked yet (a gentle offer card fades
  // in on first arrival), true = yes please, false = no thanks.
  // "Replay tour" on Profile flips it back to true, so declining is
  // never permanent.
  const [toursEnabled, setToursEnabled] = usePersistedState("toursEnabled", null);
  ...
  // Same mechanism for the pinned brew controls, which are collapsed by
  ...
  const [blendTourAxis, setBlendTourAxis] = useState(null);
  ...
  // all reset alongside the original sessions/blends so the
  ...
  const applySeedMode = (mode) => {
    ...
    // Freshly-onboarded seeds derive saved/favorites from the same
  ...
  // before it was retired. The pantry feature has since been removed
  // entirely, so any stale persisted mode just lands on Recipes.
  React.useEffect(() => {
  ...
  /* REPLAY TOUR — the Profile escape hatch, as one operation.

     A TOUR THE USER ASKED FOR DOESN'T WAIT FOR THE POEM. `arrivalDone`
     is a 4400ms stopwatch started at app mount so the FIRST tour of a
     session can't dim Home's opening sequence mid-animation. The tour
     trigger effect reads it unconditionally, so a replay tapped inside
     that window sat there doing nothing: measured at 4.1s from tap to
     callout, against 0.23s once the timer had elapsed. Reported as a
     noticeable delay on replay, and it is the whole delay.

     Nothing is being protected by then. The arrival plays ONCE per
     session — `homeArrived` in HomeScreen is a module flag, not a
     per-mount one — so by the time anyone has walked to Profile and
     tapped this, the sequence has either finished or been abandoned by
     leaving Home, and it can never play again. The stopwatch is a
     second copy of a fact HomeScreen already owns, and the two disagree
     exactly here: HomeScreen says "already arrived, don't animate"
     while App is still counting.

     Clearing it on request is the narrow fix — the gate keeps its job
     for the automatic first tour, which is the case it was written for
     and the only one where the animation is really on screen. */
  const replayTours = () => {
    setToursSeen({});
  ...
  /* Start a saved cup on the spot — no prompt, no navigation. The
     deliberate exception to "brewing asks first": the confirmation
     exists to catch an accidental commit while you're dialling
     something in, and this is a recipe you already keep. */
  const quickBrew = (blend) => {
    if (!blend) return;
    startBrew(blend, "", blend.moods || ["calm"], { minimized: true });
  };
  ...
  useEffect(() => {
    ...
    const ctx = buildAttributeContext({
      sessions, savedBlendIds, profile, journalEntries, tabVisits,
    });
  ...
  const appContent = (
    <div style={{
      ...
      <div ref={scrollRef} style={{
        ...
        {tab === "home"    && <HomeScreen   go={go} openBlend={openBlend} openCup={openCup} openInCompose={openInCompose} sessions={sessions} savedBlendIds={savedBlendIds} favoriteBlendIds={favoriteBlendIds} profile={profile} elementalsDisabled={elementalsDisabled} addJournalEntry={addJournalEntry} journalEntries={journalEntries} />}
        ...
        {tab === "profile" && <ProfileScreen go={go} openCup={openCup} sessions={sessions} savedBlendIds={savedBlendIds} seedMode={seedMode} setSeedMode={setSeedMode} profile={profile} setProfile={setProfile} resetEverything={resetEverything} startTour={replayTours} isDev={isDev} devModeEnabled={devModeEnabled} setDevModeEnabled={setDevModeEnabled} elementalsDisabled={elementalsDisabled} setElementalsDisabled={setElementalsDisabled} lodestoneCharge={lodestoneCharge} setLodestoneCharge={setLodestoneCharge} journalEntries={journalEntries} tabVisits={tabVisits} wildElementals={wildElementals} seenElementalIds={seenElementalIds} rolledElementalIds={rolledElementalIds} omenShown={omenShown} devForceGlimpse={isDev ? (() => {
      ...
      {/* Per-screen guided tour — walks the current screen's controls
          the first time you land on it (or on replay). Rendered above
          the tab bar and screen content, below the steep/detail modals. */}
      ...
      {overlay === "cup" && cupOverlayId && (
        <CupDetail
          ...
          snoozeSessionMoods={snoozeSessionMoods}
        />
      )}
