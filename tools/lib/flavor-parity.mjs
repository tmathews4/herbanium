/* ──────────────────────────────────────────────────────────────
   ...
   written, and never looked at flavors at all. That blind spot cost
   real work: sixteen flavors sat prescribed in §6 rows and absent
   ...
   First, the docs don't use a controlled vocabulary for flavor the
   way they do for effects. A §6 flavors row reads
   ...
   almost all vocabulary mismatch. So only words the app's own flavor
   vocabulary knows are compared; the rest are prose and ignored.

   Second, flavor leaf words are near-synonyms in a way effect words
...
/** Flavor families a doc's §6 rows name, and those a profile shows. */
export function flavorFamilyGaps(EXTRACTION_PROFILES) {
  const gaps = [];
...
export const flavorGapKey = g => `${g.id}:${g.family}`;
